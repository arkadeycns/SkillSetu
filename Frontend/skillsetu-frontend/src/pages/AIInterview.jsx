...
export default function AIInterview() {
  ...
  // --- RETRIEVE HANDSHAKE DATA FROM PREVIOUS PAGE ---
  ...
  // Catch the raw audio file generated by the backend!
  ...
  // --- 1. INITIAL GREETING ---
  useEffect(() => {
    // Show the message from the backend and play the REAL audio blob!
    setMessages([
      { 
        sender: 'ai', 
        text: initialQuestion, 
        audioUrl: initialAudioUrl // Automatically plays via the autoPlay prop below
      }
    ]);
  ...
  // --- 2. SENDING TO BACKEND RAG PIPELINE ---
  const handleSendAudio = async () => {
    if (!stagedAudioBlob) return;

    // Show user's audio in the chat
    ...
    // PASS THE SESSION ID TO THE BACKEND
    const aiResult = await sendAudioToAI(blobToSend, sessionId);
    setIsProcessing(false);

    // Handle the backend response (which is now an audio URL!)
    if (aiResult && aiResult.audioUrl) {
      setMessages(prev => [...prev, { sender: 'ai', text: null, audioUrl: aiResult.audioUrl }]);
        ...
        {messages.map((msg, index) => (
          <div key={index} className={`flex ${msg.sender === 'user' ? 'justify-end' : 'justify-start'} animate-fade-in-up`}>
            ...
            <div className={`max-w-[85%] md:max-w-[70%] p-5 rounded-2xl shadow-xl ${
              ...
              {/* Note: I added autoPlay so the AI's audio plays automatically when it arrives! */}
              {msg.audioUrl && (
                  <audio 
                    src={msg.audioUrl} 
                    controls 
