...
export default function Result() {
  ...
  const weaknesses =
    summary?.improvements || summary?.weaknesses || summary?.cons || [];
  ...
  useEffect(() => {
    async function fetchPlan() {
      if (sessionId) {
        const contextData = {
          ...
          strengths: strengths,
          gaps: weaknesses,
          skills: strengths, // Using strengths as current skills
        };

        const data = await getTrainingRecommendations(sessionId, contextData);
    ...
    fetchPlan();
  }, [sessionId, skill, lang, strengths, weaknesses]); // Added dependencies
          ...
          <div className="bg-slate-800 border border-slate-700 rounded-3xl p-6 md:p-10 shadow-2xl mb-8">
            ...
            {score !== "Complete" && (
              <div className="flex flex-col items-center justify-center mb-10 pb-10 border-b border-slate-700">
                <p className="text-sm text-slate-400 uppercase tracking-widest font-black mb-2">
                  Overall Score
                </p>
              ...
              {isLoadingPlan ? (
                ...
                <div className="mt-4 space-y-4">
                  <p className="text-slate-400 text-sm mb-4">
                    Focus on these topics to improve your technical profile:
                  </p>
                  ...
                  {trainingPlan.modules.map((mod, idx) => (
                    ...
                    >
                      ...
                      <div className="flex flex-wrap gap-2 mt-2">
                        {mod.skills_to_learn &&
                          mod.skills_to_learn.map((skillItem, sIdx) => (
                            <span
                              key={sIdx}
                              className="bg-blue-600/20 border border-blue-500/30 text-blue-300 text-xs px-2 py-1 rounded-md"
                            >
                              {skillItem}
                            </span>
                          ))}
